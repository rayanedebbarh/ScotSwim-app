...
(() => {
  ...
  const stylesheet =
    ...
    ':host([minimal]) .empty svg,:host([minimal]) .empty .sub{display:none}' +
    ':host([minimal]) .empty{gap:0;padding:2px}' +
    ':host([minimal]) .empty .cap{font-weight:700;letter-spacing:.06em;opacity:1}' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      // (written into the HTML itself) so it passes through unchanged.
